...
class DraftDayDialog(QDialog):
    ...
    def __init__(
        ...
        Args:
            controller: DraftDialogController instance for business logic
    ...
    def _check_resume_draft(self):
        ...
        If draft progress is detected in database (draft_in_progress=True and
        current_draft_pick > 0), shows a message box informing the user that
        the draft will resume from the saved position.

        Note:
            UI state restoration (scrolling, highlighting) happens automatically
            via refresh_all_ui() which uses controller.current_pick_index.
        """
        # Load draft state from controller
        state = self.controller.load_draft_state()

        # Check if draft was in progress
