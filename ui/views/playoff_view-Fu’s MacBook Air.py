...
class PlayoffView(QWidget):
    ...
    def refresh(self) -> None:
        ...
        print(f"[DEBUG PlayoffView] self.controller = {self.controller}")

        if not self.controller:
            print("[DEBUG PlayoffView] No controller - showing no data message")
            self._show_no_data_message()
            return

        # Check if playoffs are active
        is_active = self.controller.is_active()
        print(f"[DEBUG PlayoffView] controller.is_active() = {is_active}")

        if not is_active:
            print("[DEBUG PlayoffView] Playoffs not active - showing not active message")
            self._show_not_active_message()
            return

        print("[DEBUG PlayoffView] Playoffs are active - fetching data...")

        try:
            # Get seeding data
            print("[DEBUG PlayoffView] Calling controller.get_seeding()...")
            seeding = self.controller.get_seeding()
            print(f"[DEBUG PlayoffView] seeding = {seeding}")
