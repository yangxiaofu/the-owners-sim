...
class KickoffPlay(PlayType):
    ...
    def _simulate_kick_placement(self, personnel, kick_strategy: str, matrix: Dict) -> Dict:
        ...
        if random.random() < KickoffGameBalance.OUT_OF_BOUNDS_PENALTY_RATE * (1.0 - kicker_effectiveness):
            return {"zone": "out_of_bounds", "yards": 0}
        
        # Base kick placement with kicker effectiveness modifier
        base_success = 0.85  # 85% base chance of hitting target zone
        effectiveness_modifier = 0.9 + (kicker_effectiveness - 0.7) * 0.4  # Gentle scaling
        placement_success = base_success * effectiveness_modifier
        
        # Add variance for realism
        variance = random.uniform(1.0 - matrix["variance"] * 0.3, 1.0 + matrix["variance"] * 0.3)
    ...
    def _determine_missed_target_zone(self, kick_strategy: str) -> str:
        ...
        if kick_strategy == "deep_kick":
            ...
            return "landing_zone" if random.random() < 0.7 else "short_zone"
        elif kick_strategy == "landing_zone_kick":
            # Landing zone kick can go short or long
            return "short_zone" if random.random() < 0.6 else "end_zone"
        else:
    ...
    def _check_touchback_rules(self, kick_outcome: Dict, matrix: Dict) -> Tuple[str, int, int]:
        ...
        if zone == "landing_zone":
            ...
            goes_to_end_zone_chance = matrix["base_touchback_chance"] * 0.6
            if random.random() < goes_to_end_zone_chance:
                # Can be downed for 20-yard touchback or returned
                if random.random() < 0.5:  # 50% chance of downing vs return
    ...
    def _simulate_return_attempt(self, offense_ratings: Dict, defense_ratings: Dict,
        ...
        effectiveness_modifier = 0.8 + (return_effectiveness * 0.4)  # Scale 0.8-1.2
        return_yards = base_yards * effectiveness_modifier
        
        # Step 3: Add variance for realism
        variance = random.uniform(0.6, 1.0 + matrix["variance"] * 0.4)
        ...
        if random.random() < KickoffGameBalance.FUMBLE_ON_RETURN_RATE:
            return "fumble", 0, max(20, int(return_yards * 0.7))  # Fumble at approximate return point
        
        # Step 6: Determine final outcome
        final_yards = max(5, int(return_yards))  # Minimum 5-yard return
        final_field_position = min(95, final_yards)  # Cap at 95 (goal line)
        
        # Check for touchdown (rare but possible)
        if final_yards >= 75 and random.random() < 0.02:  # 2% TD chance on long returns
            return "touchdown", final_yards, 100
    ...
    def _calculate_return_effectiveness(self, offense_ratings: Dict, defense_ratings: Dict,
        ...
        coverage_rating = offense_ratings.get('special_teams', 70)  # Kicking team covers
        coverage_effectiveness = (coverage_rating / 100.0) * matrix["coverage_effectiveness"]
        ...
        coverage_effectiveness *= (1.0 + hang_time_factor * KickoffGameBalance.HANG_TIME_IMPACT_FACTOR)
        ...
        net_effectiveness = returner_rating / (coverage_effectiveness + 0.3)  # Avoid harsh penalties
        
        return min(1.5, max(0.3, net_effectiveness))  # Keep in reasonable range
    ...
    def _get_returner_effectiveness(self, personnel, matrix: Dict) -> float:
        ...
        if returner:
            ...
            for attribute in matrix["returner_attributes"]:
                rating = getattr(returner, attribute, 50)
                if not isinstance(rating, (int, float)):
                    rating = 50
                total_rating += rating
            
            avg_rating = total_rating / len(matrix["returner_attributes"])
            return avg_rating / 100.0  # Normalize to 0-1 range
        
        # Fallback to team special teams rating
        team_st_rating = getattr(personnel, 'special_teams_rating', 70)
        if not isinstance(team_st_rating, (int, float)):
            team_st_rating = 70
        
        return team_st_rating / 100.0
    ...
    def _calculate_kicker_effectiveness(self, personnel, kick_strategy: str, matrix: Dict) -> float:
        ...
        if kicker:
            ...
            for attribute in matrix["kicker_attributes"]:
                rating = getattr(kicker, attribute, 70)  # Kickers typically better baseline
                if not isinstance(rating, (int, float)):
                    rating = 70
                total_rating += rating
        ...
        return min(1.0, (team_st_rating + 5) / 100.0)
    ...
    def _check_breakaway_potential(self, personnel, current_yards: float, matrix: Dict) -> bool:
        ...
        returner = getattr(personnel, 'returner_on_field', None)
        if returner:
            speed = getattr(returner, 'speed', 75)
            if not isinstance(speed, (int, float)):
                speed = 75
        else:
            speed = 75  # Average speed
    ...
    def _simulate_onside_kick(self, offense_ratings: Dict, defense_ratings: Dict, matrix: Dict) -> Tuple[str, int, int]:
        ...
        hands_team_rating = defense_ratings.get('special_teams', 70)
        
        # Onside kicks favor the receiving team heavily
        kicker_effectiveness = kicker_rating / 100.0
        hands_effectiveness = (hands_team_rating / 100.0) * 1.3  # Hands team advantage
        ...
        if random.random() < recovery_chance:
            ...
            return "onside_recovery", 15, 45  # Recovered around 45-yard line
        else:
