...
Standalone wrapper for NFL game simulation that integrates the existing
FullGameSimulator with a simple event interface. This replaces the previous
...
class GameResult:
    """
    Simple game result data structure
    """
    event_name: str
    date: datetime
    away_team_id: int
    ...
    def get_summary(self) -> str:
        ...
        if self.success:
            return f"{self.event_name} - {self.away_score}-{self.home_score} ({status})"
        else:
            return f"{self.event_name} - {status}: {self.error_message}"

    def is_success(self) -> bool:
        """Check if the game simulation was successful"""
        return self.success

    def get_final_score(self) -> Dict[str, int]:
        """Get the final score as a dictionary"""
        return {
...
class GameSimulationEvent:
    """
    NFL Game simulation event that wraps the existing FullGameSimulator.

    This provides a simple interface for simulating individual NFL games
    without requiring the full calendar management system.
    """
    ...
    def __init__(self, date: datetime, away_team_id: int, home_team_id: int,
        ...
        self.duration_hours = 3.5  # NFL games typically take 3-3.5 hours
        ...
        self.event_name = f"{away_name} @ {home_name} (Week {week})"

        # Generate unique event ID
        team_ids = f"{away_team_id}_{home_team_id}"
        date_str = self.date.strftime("%Y%m%d")
        self.event_id = f"{date_str}_{team_ids}_{self.event_name.replace(' ', '_')}"

        # Track involved teams
        self.involved_teams = [away_team_id, home_team_id]
    ...
    def simulate(self) -> GameResult:
        ...
        Returns:
            GameResult: Game results in simplified format
        """
        if FULL_GAME_SIMULATOR_AVAILABLE:
            try:
                ...
                away_score = 0
                home_score = 0
                if final_score and "scores" in final_score:
                    scores = final_score["scores"]
                    away_score = scores.get(self.away_team_id, 0)
                    home_score = scores.get(self.home_team_id, 0)

                # Determine winning team ID
                winning_team_id = None
                if away_score > home_score:
                    winning_team_id = self.away_team_id
                elif home_score > away_score:
                    winning_team_id = self.home_team_id
                # winning_team_id stays None for ties
                ...
                metadata = {
                    ...
                    "game_result_object": game_result,
                    "player_stats": player_stats_for_result
                }
            ...
            winning_team_id = self.away_team_id if away_score > home_score else (
                self.home_team_id if home_score > away_score else None
            )
            ...
            return GameResult(
                ...
                metadata={
                    ...
                    "season_type": self.season_type,
                    "placeholder": True,
                    "message": f"Placeholder game simulation - Final score: {away_score}-{home_score}"
                }
            )
    ...
    def validate_preconditions(self) -> tuple[bool, Optional[str]]:
        ...
        if not self.involved_teams:
            return False, "Game must involve exactly two teams"

        if len(self.involved_teams) != 2:
            return False, "Game must involve exactly two teams"

        if self.duration_hours <= 0:
            return False, "Game duration must be positive"
